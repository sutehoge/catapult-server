...
namespace catapult { namespace api {
	...
	namespace {
		...
		struct MessagesTraits {
			static constexpr auto Request_Data_Header_Size = SizeOf32<model::FinaliazationRound>();
			...
			static auto Invoke(const RemoteFinalizationApi& api) {
				return api.messages({ FinalizationEpoch(11), FinalizationPoint(22) }, KnownShortHashes());
			}
			...
			static void ValidateRequest(const ionet::Packet& packet) {
				...
				EXPECT_EQ(FinalizationPoint(11), reinterpret_cast<const FinalizationEpoch&>(*packet.Data()));
				EXPECT_EQ(FinalizationPoint(22), reinterpret_cast<const FinalizationPoint&>(*(packet.Data() + sizeof(FinalizationEpoch))));
