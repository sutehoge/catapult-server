...
#define PLUGIN_TEST(TEST_NAME) \
	...
	PLUGIN_TEST(CanMapLinkTransaction) {
		...
		if constexpr (std::is_same_v<VotingKey, decltype(transaction.LinkedPublicKey)>) {
			...
			EXPECT_EQ(transaction.StartEpoch, FinalizationPoint(test::GetUint64(view, "startEpoch")));
			EXPECT_EQ(transaction.EndEpoch, FinalizationPoint(test::GetUint64(view, "endEpoch")));
		} else {
