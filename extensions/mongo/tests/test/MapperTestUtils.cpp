	...
	namespace {
		...
		void AssertPublicKeysSubDocument(
			...
			for (const auto& dbPublicKeyDocument : dbPublicKeysDocument.view()["publicKeys"].get_array().value) {
				...
				EXPECT_EQ(expectedPublicKey.StartEpoch, FinalizationPoint(GetUint64(dbPublicKeyDocument, "startEpoch"))) << "at " << i;
				EXPECT_EQ(expectedPublicKey.EndEpoch, FinalizationPoint(GetUint64(dbPublicKeyDocument, "endEpoch"))) << "at " << i;
				++i;
			}
